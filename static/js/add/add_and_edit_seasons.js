...
// async function checkIfAddedSeasons() {
//   const csrftoken = Cookies.get("csrftoken");

//   let is_legacy = await fetch(`/api/is-legacy/show/${window.location.href.split("/")[4]}`);
//   is_legacy = await is_legacy.json();

//   let show_response = await fetch(`/api/tv-show-detail/${window.location.href.split("/")[4]}`);
//   show_response = await show_response.json();

//   if (is_legacy) {
//     let data = {
//       id: show_response.tvdb_id,
//       base_path: show_response.file_path,
//       current_name: "none",
//       new_name: "none",
//       renamed: false,
//       type: "Season",
...
//     let new_file_path = await fetch(`/api/update-file-path/show/${show_response.id}`, {
//       method: "POST",
//       headers: {
//         "Content-Type": "application/json",
//         "X-CSRFToken": csrftoken,
...
//       body: JSON.stringify(data),
//     });
//     new_file_path = await new_file_path.json();
//   } else {
//     let response = await fetch(`/api/list-new-files/seasons`);
//     response = await response.json();

//     if (response.length > 0) {
//       for (let i of response) {
//         if (i.base_path == show_response.file_path) {
//           console.log(show_response);
//           i.name = show_response.name;
//           i.id = show_response.tvdb_id;
//           console.log(i);
//           response = await fetch(`/api/rename`, {
//             method: "POST",
//             headers: {
...
add_season_button.addEventListener("click", async (e) => {
  season_numbers = 0;
...
async function mediaInfo(current_season) {
  ...
  // this is a issue because file name does not contain the [id] for legacy added content
  const split_file_data = response.file_name
    .toLowerCase()
    .split(/\[\d+\]/)[1]
    .split(".");
